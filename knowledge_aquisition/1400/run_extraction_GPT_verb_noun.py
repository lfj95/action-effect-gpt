...
def main(models, datasets, all_shots, num_seeds, subsample_test_set, api_num_log_prob, bs, concept_relations, use_saved_results):
    """
    ...
    for model in models:
        for dataset in datasets:
            for num_shots in all_shots:
                for seed in range(num_seeds):  #range(num_seeds)
                    p = deepcopy(default_params)
                    p['model'] = model
                    p['dataset'] = dataset
                    p['seed'] = seed
                    p['num_shots'] = num_shots
                    p['expr_name'] = f"{p['model']}_{p['num_shots']}shot_{p['concept_relations']}_fact_seed{p['seed']}"
                    p['expr_name'] = p['expr_name'].replace(' ','_')
                    all_params.append(p)
...
def save_results(params_list, freeze_test_set=True):
    ...
    for param_index, params in enumerate(params_list):
        ...
        ### load data
        all_sentences, all_labels = load_dataset(params)
        ...
        np.random.seed(params['seed'])
        train_sentences, train_labels, test_sentences, test_labels = random_sampling(all_sentences, all_labels, params['num_shots'], params['seed'])
        ...
        all_responses_orig, all_prompts_orig = get_model_response(params, train_sentences, train_labels, test_sentences,
                                                        return_all_prompts=True, num_tokens_to_predict_override=60)
        all_orig_ans = []
...
#                                                         return_all_prompts=True, num_tokens_to_predict_override=60-1,
...
# =============================================================================
        ...
        ref0, ref1,ref2, ref3, ref4, ref5, ref6, ref7, ref8, ref9 = get_all_annotations(test_sentences)
        test_all_labels=[ref0, ref1, ref2, ref3, ref4, ref5, ref6, ref7, ref8, ref9]
        
        ### Get accuracy
        all_orig_ans = [ans.strip() for ans in all_orig_ans]
        # if params['model'] == 'gpt2':
        all_orig_ans = [ans.split('.')[0] for ans in all_orig_ans]


        # all_reweighted_ans = [ans.strip() for ans in all_reweighted_ans]


        accuracies = em_accuracy_helper(all_orig_ans, [test_labels])
        reweighted_accuracy = em_accuracy_helper(all_orig_ans, test_all_labels)
        

    
        print(f"metrics_dict {accuracies}")
        print(f"10_metrics_dict {reweighted_accuracy}")
        ...
        result_to_save['test_all_labels']=test_all_labels
        result_to_save['test_labels'] = test_labels
        result_to_save['prompts'] = all_prompts_orig
        result_to_save['generations_orig'] = all_responses_orig

        # result_to_save['all_responses_first'] = all_responses
        # result_to_save['all_responses_greedy'] = all_responses_greedy
        result_to_save['generations_before.'] = all_orig_ans
        # result_to_save['all_reweighted_ans'] = all_reweighted_ans
        result_to_save['10_metrics'] = reweighted_accuracy
        result_to_save['1_metrics'] = accuracies
...
def em_accuracy_helper(prediction, label):
    nlgeval=NLGEval()
    metrics_dict=nlgeval.compute_metrics(hyp_list=prediction,ref_list=label)
...
if __name__ == '__main__':
    ...
    # required arguments
    parser.add_argument('--models', dest='models', action='store',
                        default='davinci', required=False, help='name of model(s), e.g., davinci, ada, curie , babbage, gpt2, gpt2-xl')
    parser.add_argument('--datasets', dest='datasets', action='store',
                        default='action', required=False, help='name of dataset(s), e.g., agnews')
    parser.add_argument('--num_seeds', dest='num_seeds',
                        default=10, action='store', required=False, help='num seeds for the training set', type=int)
    parser.add_argument('--all_shots', dest='all_shots',
                        default=[1,2,4,8], action='store', required=False, help='num training examples to use')
    # other arguments
    ...
    parser.add_argument('--bs', dest='bs', action='store', required=False, type=int, default=15,
                        help='batch size for model queries. For OpenAI API, capped at 20. For local running, set this to max out your GPU memory.')
    parser.add_argument('--concept_relations', action='store', required=False, default="no")  
                          # causes, capable of, desires, desire of, motivated by goal, used for, made of    
    # flags
    ...
    # with open('.\saved_results\origin_qa\\action_gpt2_2shot_None_subsample_seed0.pkl', 'rb') as f:
    #     gpt2_2shot= pickle.load(f)    
    # #     print(x_gpt2['1_metrics'],x_gpt2['10_metrics'])
    with open('.\saved_results\origin_qa\\action_ada_2shot_None_subsample_seed0.pkl', 'rb') as f:
        ada_2shot = pickle.load(f)   
